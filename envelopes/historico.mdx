...
if (response.ok) {
  ...
  console.log(`=== HISTÓRICO DO ENVELOPE (${history.length} ações) ===`);
  ...
  history.forEach((action, index) => {
    ...
    // Diferentes tipos de ação com contexto adicional
    switch(action.action.toLowerCase()) {
      case 'created':
        console.log(`   📄 Envelope criado e preparado para envio`);
        break;
      case 'sent':
        console.log(`   📧 Envelope enviado para todos os destinatários`);
        break;
      case 'viewed':
        console.log(`   👀 Documento visualizado pelo destinatário`);
        break;
      case 'signed':
        console.log(`   ✍️ Documento assinado digitalmente`);
        break;
      case 'completed':
        console.log(`   ✅ Todas as assinaturas foram coletadas`);
        break;
      case 'voided':
        console.log(`   ❌ Envelope cancelado - processo interrompido`);
        break;
      case 'declined':
        console.log(`   🚫 Assinatura recusada pelo destinatário`);
        break;
      case 'expired':
        console.log(`   ⏰ Envelope expirado por falta de ação`);
        break;
      case 'reminder_sent':
        console.log(`   🔔 Lembrete automático enviado`);
        break;
      default:
  ...
  console.log(`\n=== STATUS ATUAL ===`);
  console.log(`Última ação: ${lastAction.action} em ${new Date(lastAction.creationDate).toLocaleString('pt-BR')}`);
...
if response.status_code == 200:
    ...
    print(f"=== HISTÓRICO DO ENVELOPE ({len(history)} ações) ===")
    ...
    action_descriptions = {
        ...
        'sent': 'Envelope enviado para todos os destinatários',
        'viewed': 'Documento visualizado pelo destinatário',
        ...
        'declined': 'Assinatura recusada pelo destinatário',
        'expired': 'Envelope expirado por falta de ação',
        'reminder_sent': 'Lembrete automático enviado'
    }
    ...
    for i, action in enumerate(history, 1):
        ...
        description = action_descriptions.get(action_key, f"Ação: {action['action']}")
    ...
    if history:
        ...
        print(f"\n=== STATUS ATUAL ===")
        print(f"Última ação: {last_action['action']} em {last_date.strftime('%d/%m/%Y %H:%M:%S')}")
